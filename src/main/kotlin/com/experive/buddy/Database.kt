...
import com.experive.buddy.steps.FetchableQuery
import com.experive.buddy.steps.InsertMoreStep
import com.experive.buddy.steps.InsertSetStep
import com.experive.buddy.steps.InsertValuesStep
...
interface Database {
     ...
     * @since 1.0.0
     */
    fun <R : Any> selectFrom(entityClass: TableInfo<R>): SelectWhereStep<R> {
        return selectForEntity(entityClass.enclosingType, entityClass.asterisk()).from(entityClass)
    }
     ...
     */
    fun <R : Any> selectCount(entityClass: TableInfo<R>): SelectWhereStep<R> {
        return selectForEntity(entityClass.enclosingType, count()).from(entityClass)
    }

    /**
     * Starts the creation of a select query
     *
     * Allows to specify which fields are selected, since from is not
     * specified at this time, the type of the query defaults to [Record]
     */
     ...
     * @sample selectCount
     */
    fun <R : Any> selectForEntity(
        entityClass: KClass<R>,
        vararg selectFieldOrAsterisk: Expression<*>
    ): SelectFromStep<R>

    fun <R : Any> update(entityClass: TableInfo<R>): UpdateSetStep<R>
    fun <R : Any> delete(entityClass: TableInfo<R>): DeleteWhereStep<R> {
        return deleteFrom(entityClass)
    }
