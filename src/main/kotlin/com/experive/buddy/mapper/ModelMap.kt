...
internal class ModelMap<K : Any>(private val output: KClass<K>) {
    private val fieldOrder = output.java.declaredFields
        .filter { !Modifier.isTransient(it.modifiers) }.associateBy { it.name }
        ...
        if (allArgsConstructor != null) {
            return allArgsConstructor.call(
                *allArgsConstructor.parameters
                    .map { f -> coerceInto(map[f.name], fieldOrder[f.name!!]!!.type) }
                    .toTypedArray()
            )
