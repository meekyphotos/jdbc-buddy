...
import com.experive.buddy.concat
...
internal class InsertQueryBuilderTest {
    ...
    companion object {
        ...
        fun qb(vararg selectFieldOrAsterisk: Expression<*>): SelectJoinStep<TestEntity> {
            return SelectQueryBuilder(
                mockk(),
                TestEntity::class,
                Dialect.of(""),
                *selectFieldOrAsterisk
            ).from(testRelationTable)
        }
        ...
        fun queries(): Stream<Arguments> {
            return Stream.of(
                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?)"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?)"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1).values("", 1),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?), (?, ?)"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1).values("", 1),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?), (?, ?)"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1).returning(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?)"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1).returning(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) RETURNING *"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1).returning(id),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?)"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1).returning(id),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) RETURNING id"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1).onConflictDoNothing(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) ON CONFLICT DO NOTHING"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1).onConflictDoNothing(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) ON CONFLICT DO NOTHING"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1).onDuplicateKeyIgnore(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) ON CONFLICT DO NOTHING"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1).onDuplicateKeyIgnore(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) ON CONFLICT DO NOTHING"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).values("", 1).onDuplicateKeyIgnore().returning(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) ON CONFLICT DO NOTHING"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).values("", 1).onDuplicateKeyIgnore().returning(),
                    "INSERT INTO test_entity (name, field_name) VALUES (?, ?) ON CONFLICT DO NOTHING RETURNING *"
                ),

                Arguments.of(
                    h2Qb().columns(name, fieldName).select(qb("name".asExpression(), trId)),
                    "INSERT INTO test_entity (name, field_name) " +
                        "SELECT ?, ${testRelationTable.alias}.id FROM test_relation ${testRelationTable.alias}"
                ),
                Arguments.of(
                    pgQb().columns(name, fieldName).select(qb("name".asExpression(), trId)),
                    "INSERT INTO test_entity (name, field_name) " +
                        "SELECT ?, ${testRelationTable.alias}.id FROM test_relation ${testRelationTable.alias}"
                ),
        ...
        fun executionQueries(): Stream<Arguments> {
            return Stream.of(
                Arguments.of(
                    { qb: InsertQueryBuilder<TestEntity> ->
                        qb
                ...
                Arguments.of(
                    { qb: InsertQueryBuilder<TestEntity> ->
                        qb
                            .columns(name, fieldName)
                            .values("e", 1).onConflictDoNothing()
                    }, 1
                ),
                Arguments.of(
                    { qb: InsertQueryBuilder<TestEntity> ->
                        qb
                            .columns(name, fieldName)
                            .values("Michele", 1).onConflictDoNothing()
                    }, 0
                ),
                Arguments.of(
                    { qb: InsertQueryBuilder<TestEntity> ->
                        qb
                            .columns(name, fieldName)
                            .values("d", 1).onDuplicateKeyIgnore()
                    }, 1
                ),
                Arguments.of(
                    { qb: InsertQueryBuilder<TestEntity> ->
                        qb
                            .columns(name, fieldName)
                            .values("d", 1).onDuplicateKeyIgnore().returning()
                    }, 1
                ),
                Arguments.of(
                    { qb: InsertQueryBuilder<TestEntity> ->
                        qb
                            .columns(name, fieldName)
                            .select(qb(concat("name".asExpression(), trId cast "VARCHAR"), trId))
                    }, 5
                ),
            )
        }
